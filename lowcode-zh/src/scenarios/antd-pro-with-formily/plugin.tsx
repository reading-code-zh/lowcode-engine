    ...
    return {
      ...
      async init() {
        ...
        project.openDocument(
          getProjectSchemaFromLocalStorage('antd-pro-with-formily').componentsTree?.[0] || schema,
        );
      },
    };
  };
    ...
    return {
      ...
      async init() {
        ...
        skeleton.add({
          ...
          content: <Button onClick={() => saveSchema('next-pro')}>保存到本地</Button>,
        });
        skeleton.add({
          name: 'resetSchema',
          area: 'topArea',
          ...
          content: <Button onClick={() => resetSchema('next-pro')}>重置页面</Button>,
        });
    ...
    return {
      ...
      async init() {
        ...
        skeleton.add({
          ...
          content: (
            <Button type="primary" onClick={() => preview('antd-pro-with-formily')}>
              预览
            </Button>
          ),
        });
